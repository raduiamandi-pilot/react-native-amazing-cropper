...
class CropperPage extends Component {
    constructor(props) {
        ...
        this.topOuter = undefined;
        this.leftOuter = undefined;
        this.bottomOuter = undefined;
        this.rightOuter = undefined;
        this.W = this.props.COMPONENT_WIDTH;
        this.H = this.props.COMPONENT_HEIGHT - Q;
        this.onCancel = () => {
            this.props.onCancel();
        };
        this.getTopOuterStyle = () => {
            return {
        ...
        this.isAllowedToMoveTopSide = (gesture) => {
        ...
        this.isAllowedToMoveLeftSide = (gesture) => {
        ...
        this.isAllowedToMoveBottomSide = (gesture) => {
            return (this.state.bottomPosition.y._offset + gesture.dy <= this.props.COMPONENT_HEIGHT - this.state.BOTTOM_LIMIT &&
                this.state.topPosition.y._offset + this.props.BORDER_WIDTH + 1 < this.state.bottomPosition.y._offset + gesture.dy);
        };
        this.isAllowedToMoveRightSide = (gesture) => {
            return (this.state.rightPosition.x._offset + gesture.dx <= this.props.COMPONENT_WIDTH - this.state.RIGHT_LIMIT &&
                this.state.leftPosition.x._offset + this.props.BORDER_WIDTH + 1 < this.state.rightPosition.x._offset + gesture.dx);
        };
        this.isAllowedToMove = (position, gesture) => {
        ...
        this.initSidePanResponder = (position) => {
        ...
        this.initRectanglePanResponder = () => {
            return PanResponder.create({
                onStartShouldSetPanResponder: () => !this.isRectangleMoving,
                onPanResponderMove: (_event, gesture) => {
        ...
        this.initCornerPanResponder = (pos1, pos2) => {
            return PanResponder.create({
                onStartShouldSetPanResponder: () => !this.isRectangleMoving,
                onPanResponderMove: (_event, gesture) => {
        ...
        this.setCropBoxValues = ({ TOP_VALUE, LEFT_VALUE, BOTTOM_VALUE, RIGHT_VALUE, }) => {
            this.setState({
                TOP_VALUE,
                LEFT_VALUE,
                BOTTOM_VALUE,
        ...
        this.setCropBoxRotation = (rotation) => {
        ...
        this.onRotate = () => {
            ...
            const { TOP_LIMIT, LEFT_LIMIT, BOTTOM_LIMIT, RIGHT_LIMIT, DIFF } = getCropperLimits(imageWidth, imageHeight, rotation, W_INT, H_INT, this.W, this.H, this.props.BORDER_WIDTH, Q);
            this.rotate90();
            this.setCropBoxLimits({ TOP_LIMIT, LEFT_LIMIT, BOTTOM_LIMIT, RIGHT_LIMIT });
            ...
            const startPositionAfterRotationY = ((imageWidthBeforeRotation - startPositionBeforeRotationX - rectangleWidthBeforeRotation) * imageHeightAfterRotation) /
        ...
        this.cropImage = (uri, cropData, garbageUris) => ImageEditor.cropImage(uri, cropData)
            .then(croppedUri => {
        ...
        this.onDone = () => {
            ...
            ImageResizer.createResizedImage(this.props.imageUri, imageWidth, imageHeight, 'JPEG', 100, Platform.OS === 'ios' ? 0 : this.state.rotation, undefined, false, {
                ...
                if (Platform.OS === 'ios' && this.state.rotation !== 0) {
                    ImageRotate.rotateImage(res.uri, this.state.rotation, (uri) => this.cropImage(uri, cropData, [res.uri, uri]), (err) => this.props.onError(err));
                }
                else {
        ...
        const { TOP_LIMIT, LEFT_LIMIT, BOTTOM_LIMIT, RIGHT_LIMIT, DIFF } = getCropperLimits(imageWidth, imageHeight, props.initialRotation, W_INT, H_INT, this.W, this.H, BORDER_WIDTH, Q);
        ...
        const bottomOuterPosition = new Animated.ValueXY({ x: LEFT_VALUE - BORDER_WIDTH, y: COMPONENT_HEIGHT - BOTTOM_VALUE });
        const bottomOuterPanResponder = PanResponder.create({ onStartShouldSetPanResponder: () => false });
        const rightOuterPosition = new Animated.ValueXY({ x: COMPONENT_WIDTH - RIGHT_VALUE, y: TOP_VALUE - BORDER_WIDTH });
        ...
        const bottomPosition = new Animated.ValueXY({ x: LEFT_VALUE - BORDER_WIDTH, y: COMPONENT_HEIGHT - BOTTOM_VALUE });
        ...
        const bottomLeftPosition = new Animated.ValueXY({ x: LEFT_VALUE - BORDER_WIDTH, y: COMPONENT_HEIGHT - BOTTOM_VALUE });
        ...
        const topRightPosition = new Animated.ValueXY({ x: COMPONENT_WIDTH - RIGHT_VALUE, y: TOP_VALUE - BORDER_WIDTH });
    ...
    render() {
        return (<Cropper imageUri={this.props.imageUri} // 'https://3.imimg.com/data3/SN/NO/MY-10244508/vertical-building-parking-500x500.jpg'
         footerComponent={this.props.footerComponent} getTopOuterStyle={this.getTopOuterStyle} getLeftOuterStyle={this.getLeftOuterStyle} getBottomOuterStyle={this.getBottomOuterStyle} getRightOuterStyle={this.getRightOuterStyle} getTopLeftStyle={this.getTopLeftStyle} getBottomLeftStyle={this.getBottomLeftStyle} getBottomRightStyle={this.getBottomRightStyle} getTopRightStyle={this.getTopRightStyle} getTopSideStyle={this.getTopSideStyle} getLeftSideStyle={this.getLeftSideStyle} getBottomSideStyle={this.getBottomSideStyle} getRightSideStyle={this.getRightSideStyle} getRectangleStyle={this.getRectangleStyle} getImageStyle={this.getImageStyle} onDone={this.onDone} onRotate={this.onRotate} onCancel={this.onCancel} topOuterPanResponder={this.state.topOuterPanResponder} leftOuterPanResponder={this.state.leftOuterPanResponder} bottomOuterPanResponder={this.state.bottomOuterPanResponder} rightOuterPanResponder={this.state.rightOuterPanResponder} topPanResponder={this.state.topPanResponder} leftPanResponder={this.state.leftPanResponder} bottomPanResponder={this.state.bottomPanResponder} rightPanResponder={this.state.rightPanResponder} topLeftPanResponder={this.state.topLeftPanResponder} bottomLeftPanResponder={this.state.bottomLeftPanResponder} bottomRightPanResponder={this.state.bottomRightPanResponder} topRightPanResponder={this.state.topRightPanResponder} rectanglePanResponder={this.state.rectanglePanResponder} topOuterRef={ref => (this.topOuter = ref)} leftOuterRef={ref => (this.leftOuter = ref)} bottomOuterRef={ref => (this.bottomOuter = ref)} rightOuterRef={ref => (this.rightOuter = ref)} COMPONENT_WIDTH={this.props.COMPONENT_WIDTH} COMPONENT_HEIGHT={this.props.COMPONENT_HEIGHT} W={this.W} H={this.H}/>);
    }
}
